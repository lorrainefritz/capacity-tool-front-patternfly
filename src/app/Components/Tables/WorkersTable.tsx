...
export const WorkersTable: React.FC<{
  ...
  // In this example, we wrap all but the 1st column and make the 1st column sortable just to demonstrate.
  return (
    <React.Fragment>
        ...
        <Tbody>
          {sortedWorkers.map((worker, rowIndex) => (
            <Tr key={rowIndex}>
              ...
              ) : worker.hightRequestOrMemory === false && worker.mediumRequestOrMemory === true ? (
                <Td>
                  <Icon status="warning">
              ...
              <Td dataLabel={columnNames.displayCurrentCpu}>{worker.displayCurrentCpu}</Td>
              <Td dataLabel={columnNames.percentageCurrentCpu}>{worker.percentageCurrentCpu}</Td>
              <Td dataLabel={columnNames.requests_cpu}>{worker.requests_cpu}</Td>
              <Td dataLabel={columnNames.percentageRequestCpu}>{worker.percentageRequestCpu}</Td>
              <Td dataLabel={columnNames.limits_cpu}>{worker.limits_cpu}</Td>
              <Td dataLabel={columnNames.percentageLimitCpu}>{worker.percentageLimitCpu}</Td>
              <Td dataLabel={columnNames.displayCurrentMemory}>{worker.displayCurrentMemory}</Td>
              <Td dataLabel={columnNames.percentageCurrentMemory}>{worker.percentageCurrentMemory}</Td>
              <Td dataLabel={columnNames.requests_memory}>{worker.requests_memory}</Td>
              <Td dataLabel={columnNames.percentageRequestMemory}>{worker.percentageRequestMemory}</Td>
              <Td dataLabel={columnNames.limits_memory}>{worker.limits_memory}</Td>
              <Td dataLabel={columnNames.percentageLimitMemory}>{worker.percentageLimitMemory}</Td>
            </Tr>
          ))}
        </Tbody>
        <Thead>
